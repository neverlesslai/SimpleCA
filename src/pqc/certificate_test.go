...
//ParsePKIXPublicKey 解析 PKIX 中的公钥，ASN.1 DER 形式。
//  编码的公钥是 SubjectPublicKeyInfo 结构
// (see RFC 5280, Section 4.1).
//
// 它返回一个 *rsa。公钥，*dsa。PublicKey， *ecdsa.公钥，或ed25519.公钥。将来可能会支持更多类型。
//
// 这种密钥通常编码在“公钥”类型的PEM块中。
...
var debugAllowSHA1 = Get("x509sha1") == "1"
...
//signingParamsForPublicKey 返回用于使用 priv 进行签名的参数。如果请求的SigAlgo不为零，则它将覆盖默认的签名算法。
	...
	for _, details := range signatureAlgorithmDetails {
		if details.algo == requestedSigAlgo {
			if details.pubKeyAlgo != pubType {
				err = errors.New("x509: 请求的签名算法与私钥类型不匹配")
				return
			}
			sigAlgo.Algorithm, hashFunc = details.oid, details.hash
			if hashFunc == 0 && pubType != Ed25519 {
				err = errors.New("x509: 无法使用请求的哈希函数进行签名")
				return
			}
...
//  - UnknownExtKeyUsage
//
// 证书由父级签名。如果父证书等于模板，则证书是自签名的。
// 参数 pub 是要tbscertificates的公钥， PRIV 是签名者的私钥。
//
// 返回的片是采用 DER 编码的证书
//
// 当前支持的密钥类型为 *rsa。PublicKey， *ecdsa.公钥和ed25519.公钥。
// pub 必须是受支持的密钥类型，并且 priv 必须是加密、具有受支持的公钥的签名者。
//
//
